...
exports.ViewProtect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    res.status(401).render("form/login", { message: "You are not logged in!" });
    // return next(new AppError("You are not logged in!", 401));
    return 0;
  }
  ...
  if (!currentUser) {
    res.status(401).render("form/login", { message: "The user not exist!." });
    // return next(new AppError("The user not exist!."), 201);
    return 0;
  }
  // 4)
  ...
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    ...
    res.status(401).render("form/login", {
  ...
  // await Faculty.
  req.user = currentUser;
  var data = await Faculty.findOne(
    { userId: currentUser._id },
    "name position school qualification field_of_teaching image username"
  );
  req.user.data = data;
  if (data.name) {
    if (data.name.split(" ")[0].length > 3) {
      data.namep = data.name.split(" ")[0];
    } else {
      data.namep = data.name.split(" ")[1];
    }
  }
...
exports.isloggedIn = async (req, res, next) => {
  ...
  try {
    ...
    res.locals.data = data;
    if (req.originalUrl == "/facultyzone/login") {
      res.status(302).redirect("/facultyzone/");
      return 0;
    }
    return next();
  } catch (error) {
